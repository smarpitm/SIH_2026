...
export default function InstrumentDetailPage() {
  ...
  if (loading) {
    return <div className="py-12 text-center text-sm text-zinc-500">Loading instrument…</div>;
  }
  ...
  if (notFound || !instrument) {
    return (
      <div className="rounded-xl border border-amber-300 bg-amber-50 p-6 text-sm text-amber-900 dark:border-amber-800/50 dark:bg-amber-950/30 dark:text-amber-200">
        Instrument not found — it may belong to another trader or the link is stale.
      </div>
    );
  }
  ...
  const fields: [string, string][] = [
    ...
    ["Make / Model", `${instrument.make} ${instrument.model}`],
    ["Capacity", instrument.capacity],
    ["District", instrument.district],
    ["Address", instrument.address],
    ["Registered", new Date(instrument.createdAt).toLocaleDateString()],
  ];
    ...
    <div className="mx-auto w-full max-w-lg space-y-6">
      <div className="no-print flex items-center justify-between">
        ...
        <button
          ...
          className="rounded-lg border border-zinc-200 bg-white px-3 py-1.5 text-xs font-semibold text-zinc-800 shadow-sm hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-200 dark:hover:bg-zinc-800"
        >
          🖨 Print
        </button>
      </div>
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <div className="no-print mt-4 border-t border-zinc-100 pt-4 dark:border-zinc-800">
          <Link
            ...
            className="inline-flex w-full items-center justify-center rounded-lg bg-zinc-900 px-4 py-2.5 text-xs font-semibold text-white shadow-sm hover:bg-zinc-800 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-100"
          >
