...
1. **Critical (Sprint 1 - Immediate) — ✅ COMPLETED 2026-09-06 (see §3.1)**:
   - [x] Fix offline verification in browser: replace `Buffer` in `lib/crypto/jws.ts` and `lib/crypto/qr.ts` with browser-compatible `Uint8Array` / `atob` / `TextDecoder` (Findings #80, #92, #114).
   - [x] Fix `/verify/offline` key sync endpoint to point to `/api/v1/public/jwks` (Finding #91).
   - [x] Fix resubmission of FAILED/REJECTED applications in `submit/route.ts` to upsert/update existing `Schedule` instead of crashing on unique constraint (Finding #106).
   - [x] Add `WHERE "certId" ~ '^PRM-CERT-[0-9]{4}-[0-9]{5}$'` in `lib/crypto/issue.ts` to prevent cast crashes on non-numeric cert IDs (Finding #108).
   - [x] Complete `POST /api/v1/certificates/issue` to transition application to `CERT_ISSUED` (Finding #107).
   - [x] Revoke `RefreshFamily` in PostgreSQL on logout (Finding #93).
   - [x] Require `district` enum on TRADER registration (Finding #94).
   - [x] Fix `useAuthStore` token update in `components/api-client.ts` (Finding #71).

2. **Next Immediate Actions (Sixth Pass Discoveries)**:
   - Restore the trader reschedule flow: collect a date in `app/trader/applications/[id]/page.tsx` and send the mandatory `newDate` (Finding **#117 — High**).
   - Fail closed in `POST /instruments` when the session has no district (Finding **#118 — Medium**).
   - Group by `u."id", u."name"` and add upper month bound in `officerProductivity` query (Findings #99, #116, **#119 — Medium**).
   - Wrap schedule reassignment transitions in a database transaction (Finding **#121 — Low**).
   - Apply-flow cleanup: comments and dates (Finding **#120 — Low**).
